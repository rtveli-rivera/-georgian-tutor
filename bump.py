...
sw.js (kartuli-vN). Bumping the service-worker cache is what makes browsers
notice a new release and show the in-app "new version ready" prompt.

Usage:
    py bump.py            # patch bump: 1.0.0 -> 1.0.1  (and sw v1 -> v2)
    py bump.py minor      # 1.0.0 -> 1.1.0
    py bump.py major      # 1.0.0 -> 2.0.0
"""
import re
import sys
...
SW_RE = re.compile(r"(const CACHE = 'kartuli-v)(\d+)(';)")
...
def main():
    ...
    print(f"SW cache    -> kartuli-{sw_ver}")
    print("\nNow deploy, e.g.:")
